...
def armstrong_nums(iter_val)
  if (iter_val == 1)  then (10**(iter_val - 1)...10**iter_val).filter { |x_value| armstrong?(x_value) }
  else (10**(iter_val - 1)...10**iter_val).filter { |x_value| armstrong?(x_value) }.concat armstrong_nums(iter_val - 1)
end
